  ...
  <div>
    <div class="block md:flex relative">
      ...
      <div class="w-[100%] md:w-[80%]">
        ...
        <!-- <h1 class="hidden col-span-2 md:flex absolute left-1/2 top-12 md:top-20 text-3xl translate-x-[-50%] p-1 opacity-[.8] rounded-lg bg-gray-600 text-white z-10 text-center">{{ datas.abbreviation }}</h1> -->
...
export default {
  ...
  methods: {
    ...
    async handleFileChange(event) {
      ...
      try {
        const response = await axios.post('http://innodip.rw:8004/api/industry/upload/logo/'+this.datas._id, formData, {
          headers: {
    ...
    async handleBannerChange(event) {
      ...
      if(this.selectedFile!=null){
      try {
        const response = await axios.post('http://innodip.rw:8004/api/industry/upload/banner/'+this.datas._id, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
    ...
    sendData() {
      ...
      apiService.handleForm('industry/update/profile/'+this.datas._id,serializedData).then(
