<template>
    <div class="block md:flex relative">
        ...
        <div class="grid p-4">
            <div v-if="isLoaded">
                <form id="formData" @submit.prevent="sendData" enctype="multipart/form-data">
                    <div class="grid grid-cols-8 w-full">
                        ...
                        <div class="col-span-8 md:col-span-4 w-full">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                                <FormInput placeholder="First name" label="First name" inputType="text"  required=true small=false name="fname" :value="datas.fname"></FormInput>
                                <FormInput placeholder="Last name" label="Last name" inputType="text"  required=true small=false name="lname" :value="datas.lname"></FormInput>
                                <FormInput placeholder="Telphone" label="Telphone" inputType="text"  required=true small=false name="phone" :value="datas.phone"></FormInput>
                                <FormInput placeholder="Email Address" label="Email Address" inputType="email"  required=true small=false name="email" :value="datas.email"></FormInput>
                                <FormInput placeholder="Date of Birth" label="Date of Birth" inputType="date"  required=true small=false name="dateOfBirth" :value="datas.dateOfBirth"></FormInput>
                                <FormInput placeholder="National Identification Number" label="National Identification Number" inputType="text"  required=true small=false name="nid" :value="datas.nid"></FormInput>
                            </div>
                            <div class="grid  grid-cols-1 md:grid-cols-2 gap-4 w-full">
                                <FormInput placeholder="Address" label="Address" inputType="text"  required=true small=false name="address" :value="datas.address"></FormInput>
                            </div>
                            <div class="grid  grid-cols-1 md:grid-cols-2 gap-4 w-full">
                                <FormInput placeholder="Biography" label="Biography" inputType="textarea"  required=true small=false name="bio" :value="datas.biography"></FormInput>
                            </div>
                        </div>
                    </div>
                    
                    <div class="flex items-center justify-between" v-if="isLoaded">
                        <FormButton type="submit" label="Save Profile" bstyle="primary"></FormButton>
                        <router-link to="/seeker/profile/education"><FormButton type="button" label="Education &rarr;" bstyle="normal"></FormButton></router-link>
                    </div>
                </form>
    ...
    export default {
        ...
        methods:{
            sendData(){
                ...
                apiService.updateProfile(serializedData).then(
                    console.log('sent')
                );
            }
        }
