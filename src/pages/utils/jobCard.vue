<template>
    <router-link :to="`${router+'/'+job.id}`" class="card-hover grid grid-cols-1 md:grid-cols-10 relative">
        <span class="md:absolute top-3 right-3 border border-1 px-2 py-1 rounded-lg bg-gray-400 text-white">{{ job.category }}</span>
        <div class="col-span-2 flex justify-center justify-start">
            <div class="hidden md:flex w-44 h-44 md:w-32 md:h-32 items-center justify-center bg-gray-200 border border-gray-300 rounded-lg overflow-hidden"><img :src="job.logo" alt=""></div>
        </div>
