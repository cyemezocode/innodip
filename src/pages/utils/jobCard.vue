<template>
  <router-link
    :to="`${router + '/' + job._id}`"
    class="card-hover grid grid-cols-1 md:grid-cols-10 relative"
  >
    <span
      class="md:absolute top-3 right-3 border border-1 px-2 py-1 rounded-lg bg-gray-400 text-white"
      >{{ job.category.industryCategoryName }}</span
    >
    <div class="col-span-2 flex justify-center justify-start">
      ...
      >
        <img :src="baseUrl + job.provider.industryLogo" alt="" />
      </div>
    </div>
    <div class="w-full col-span-8 p-3 pt-0">
      ...
      <p
        ...
        v-html="fewWords(job.description, 20)"
      ></p>
    </div>
  </router-link>
</template>
